...
└── static/
    ├── config.js           # NocoBase 配置（API URL、Token）
    ├── nocobase-api.js     # API 客户端封装
    ├── parse.js            # 自然语言解析器
    ├── utils.js            # 统计计算工具函数
    └── chart.js            # Chart.js 库
```
...
```
config.js → utils.js → nocobase-api.js → parse.js → 页面脚本
```
...
```bash
# 启动带 AI 代理的本地服务器（端口 18080）
cd /Users/szd/Documents/Code/accounting-app/web
python3 server.py 18080

# 浏览器访问
# 首页：http://localhost:18080/index.html
# AI 对话：点击右下角悬浮按钮
```

**本地测试流程：**
1. 修改代码后，先在本地 `python3 server.py 18080` 启动测试
2. 浏览器打开 http://localhost:18080/index.html 验证功能
3. 测试通过后，同步到 NAS 或云端

### 8.2 本地代理说明

`server.py` 提供两个功能：
1. **静态文件服务**：提供 HTML/CSS/JS 文件
...
- Token 有效期 1 年（至 2027-05-05）
- 到期后需重新生成，修改 `web/static/config.js` 中的 `API_TOKEN`
- 同时需要更新 `web/server.py` 中的 `NOCOBASE_CONFIG['API_TOKEN']`
