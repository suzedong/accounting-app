...
> **当前状态（2026-07-09）：暂无进行中的活跃设计条目**。本文件收录了历史上进行过的专项设计与重构方案，供回溯每个设计的动机、决策、落地清单：
>
> | § | 主题 | 状态 |
> |---|---|---|
> | 1 · 1.10 | AI Agent 架构重构 + Agent Session 架构 | ✅ 已完成（2026-06-05） |
> | 2 | OCR Python 管理重设计 | ✅ 已完成（2026-06-04） |
>
> 当前活跃架构以 [CODE_WIKI.md](../CODE_WIKI.md) 为准；同步方案参见 [CODE_WIKI.md §9 Turso 同步机制](../CODE_WIKI.md#9-turso-同步机制)。未来启动新的重构 / 设计时会新增章节到本文件顶部。
...
2. **推理链可视化** — 每条消息展示完整执行过程（意图识别 → 执行操作 → 最终回复）
3. **分层架构** — UI 层 / Store 层 / Agent 层 / 工具层
...
interface ChatMessage {
  ...
  render?: string;        // text | card | chart | list | followUp | correction | delete | candidateSelect | tripCandidateSelect 等
  steps?: Step[];         // 推理链步骤
...
推理链结构：`意图识别` 步骤记录 LLM 返回的 tool call 及字段来源；`执行操作` 步骤记录工具执行结果；后续的 LLM 回填步骤 title 由 LLM 自身生成（如"最终回复"、"回复用户"等文案），代码不强制枚举。
...
interface Step {
  id: string;
  title: string;          // '意图识别' | '执行操作' | 工具/LLM 生成的自定义标题
...
| 文件 | 变更 |
|---|---|
| `src/components/chat/ChatWidget.vue` | 瘦身为 UI 层，专注消息列表/输入/欢迎页/设置面板 |
...
- 存储到 SQLite `app_config` 表，key 为平台后缀形式：`active_python_path_macos` / `active_python_path_windows` / `active_python_path_linux`（避免 Turso 同步时 macOS/Windows 互相覆盖路径）；旧的单 key `active_python_path` 由 `AppConfig::new()` 启动时自动迁移到当前平台的新 key
...
| `install_bundled_python` | `session_id` | 安装 Python 3.12 到应用数据目录（shell 脚本：ditto + codesign） |
...
使用 `active_python_path_<os>` 配置（`_macos` / `_windows` / `_linux`）来确定用哪个 Python，不再依赖 `detect_python()` 的自动选择。
