...
>
> **§3「NocoBase 全量对比同步」已废弃**（2026-07-09），仅作历史归档保留。当前同步方案参见 [CODE_WIKI.md §9 Turso 同步机制](../CODE_WIKI.md#9-turso-同步机制)。
...
## 3. NocoBase 全量对比同步（已废弃 · 历史归档）

> 最后更新：2026-06-10
> **状态：⛔ 已废弃**（2026-07-09）
>
> 本节记录的 NocoBase 全量对比同步方案已随阶段 4.6 迁移整体移除，代码层面已删除 `src-tauri/src/db/nocobase/`、`src-tauri/src/commands/sync.rs`、`src-tauri/src/db/sync_log.rs` 与前端相关 API。当前同步方案为 **Turso Embedded Replica**，详见 [CODE_WIKI.md §9 Turso 同步机制](../CODE_WIKI.md#9-turso-同步机制)。
>
> 数据库层面保留了 6 个 NocoBase 遗留列（`synced / nocobase_id / nocobase_updated_at / retry_count / last_error / last_synced_at`）以及 `sync_log` 表，作为历史遗留结构，业务代码**不再读写**。
>
> 以下内容仅作为演进史料保留。
...
- `retry_count`：同步重试次数（上限 3 次）
- `last_error`：最后一次同步错误信息
...
interface TableSyncResult {
  records_pushed: number;     // 记账记录推送数
  records_pulled: number;     // 记账记录拉取数
  records_deleted: number;    // 记账记录本地删除数（云端已删除）
  records_conflicts: number;  // 记账记录冲突数（已自动处理）
  trips_pushed: number;       // 差旅补助推送数
  trips_pulled: number;       // 差旅补助拉取数
  learning_pushed: number;    // 学习数据推送数
  learning_pulled: number;    // 学习数据拉取数
  total_pushed: number;       // 总推送数
  total_pulled: number;       // 总拉取数
  total_deleted: number;      // 总删除数
  total_conflicts: number;    // 总冲突数
  errors: string[];           // 错误信息
}
```
